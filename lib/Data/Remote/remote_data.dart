...
class RemoteDataSourceX {
  ...
  static Future<(T data, String? message)> _call<T>(
    ...
    if((error?.errorCode??-1)!=ErrorCodesX.unauthorized && (param.page == null || param.page == 1)) {
      result = _useLocal<T>(param.localCacheKey, result);
    }

    // Return the result if it's not null
    if (result != null) {
      String? message = (result is Map && result[RemoteKeyX.infoMessage] is String)
  ...
  /// Get
  static Future<(T data, String? message)> get<T>(String url, {DataSourceParamX? param}) async {
    param ??= DataSourceParamX(); // Use default parameters if none provided
    return await _call(
      url,
      () async{
  ...
  /// Post
  static Future<(T data, String? message)> post<T>(String url,
      {Map<String, dynamic>? body, DataSourceParamX? param}) async {
    param ??= DataSourceParamX(); // Use default parameters if none provided
    return await _call(
      url,
          () async {
  ...
  /// Put
  static Future<(T data, String? message)> put<T>(String url,
      {Map<String, dynamic>? body, DataSourceParamX? param}) async {
    param ??= DataSourceParamX(); // Use default parameters if none provided
    return await _call(
      url,
  ...
  /// Delete
  static Future<(T data, String? message)> delete<T>(String url,
      {Map<String, dynamic>? body, DataSourceParamX? param}) async {
    param ??= DataSourceParamX(); // Use default parameters if none provided
    return await _call(
      url,
