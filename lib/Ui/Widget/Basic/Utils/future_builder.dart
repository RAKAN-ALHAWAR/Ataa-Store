...
class FutureBuilderX<T> extends StatefulWidget {
  ...
  @override
  State<FutureBuilderX<T>> createState() => _FutureBuilderXState<T>();
}

class _FutureBuilderXState<T> extends State<FutureBuilderX<T>> {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          Future.microtask(() => widget.onError?.call());
          if (widget.isHideIfError) {
            return const SizedBox();
          } else {
            return ErrorView(
              error: snapshot.error,
              onTapButton: onRefresh,
            );
          }
        }

        /// Success State
        Future.microtask(() => widget.onSuccess?.call());
