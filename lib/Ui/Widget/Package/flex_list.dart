...
class FlexList extends MultiChildRenderObjectWidget {
  ...
  /// The text direction to use for layout alignment.
  /// If null, it will use the ambient text direction from the context.
...
class RenderFlexList extends RenderBox
  ...
  /// The text direction used for layout alignment
  TextDirection get textDirection => _textDirection;
  TextDirection _textDirection;

  /// Sets the text direction and marks that
  /// the [RenderBox] needs to get relayed out.
  ...
  void performLayout() {
    ...
    for (int i = 0; i < rows.length; ++i) {
      ...
      // Get all children in this row
      List<RenderBox> rowChildren = <RenderBox>[];
      List<double> childWidths = <double>[];

      var currentChild = child;
      while (currentChild != null) {
        final _FlexListParentData childParentData =
            currentChild.parentData! as _FlexListParentData;

        if (childParentData._rowIndex != i) {
          break;
        }

        rowChildren.add(currentChild);

        final lastItemPadding =
            itemNumber + 1 == row.childNumber && i != 0 ? horizontalSpacing : 0;

        final double finalChildWidth;
        final equalWidth = (constraints.maxWidth -
                (horizontalSpacing * (row.childNumber - 1))) /
            row.childNumber;
        ...
        childWidths.add(finalChildWidth);

        var consts = constraints.tighten(width: finalChildWidth);
        currentChild.layout(consts);

        currentChild = childParentData.nextSibling;
        itemNumber++;
      }

      // Position the children based on text direction
      if (textDirection == TextDirection.ltr) {
        // Left-to-right layout
        double offsetX = 0.0;
        for (int j = 0; j < rowChildren.length; j++) {
          final childParentData =
              rowChildren[j].parentData! as _FlexListParentData;
          childParentData.offset = Offset(offsetX, offsetY);
          offsetX += childWidths[j] + horizontalSpacing;
        }
      } else {
        // Right-to-left layout
        double offsetX = constraints.maxWidth;
        for (int j = 0; j < rowChildren.length; j++) {
          final childParentData =
              rowChildren[j].parentData! as _FlexListParentData;
          offsetX -= childWidths[j];
          childParentData.offset = Offset(offsetX, offsetY);
          offsetX -= horizontalSpacing;
        }
      }

      // Update child to first child of next row
      child = rowChildren.isNotEmpty &&
              rowChildren.last.parentData is _FlexListParentData
          ? (rowChildren.last.parentData! as _FlexListParentData).nextSibling
          : null;
